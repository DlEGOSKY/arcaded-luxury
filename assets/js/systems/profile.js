...
//   app.stats, app.credits, app.highScores, app.agentName
//   app.getRankName(level), app.calculateRank(gameId, score)
//   app.audio.playClick(), app.audio.playWin(n)
//   app.save(), app.updateUI()
//   app.addNotif(icon, title, body, type)
//
// IMPORTANTE: los onclick inline de las tarjetas y botones llaman a
// `window.app.setAvatar(...)`, `window.app.setTitle(...)`,
// `window.app.closeProfile()`, `window.app.editAgentName()`. Como
// `main.js` conserva esos nombres como proxies, la API publica
// queda intacta.
//
// BUG FIX aplicado: la version anterior incluia `${globalStatsHTML}`
// en el template pero `globalStatsHTML` nunca estaba declarada, lo
// que lanzaba ReferenceError al abrir el perfil. La seccion
// "ESTADISTICAS GLOBALES" dead se eliminio del template; puede
// reponerse cuando se calcule el HTML realmente.
...
export function show(app) {
    ...
    const unlockedCount = safeRender(
        () => CONFIG.ACHIEVEMENTS.filter(a => a.check(ctx, app)).length,
        0
    );
    ...
    modal.innerHTML = `
        <div class="profile-v2">
            ...
            ${safeRender(() => renderSection('LOGROS', `<div class="pv2-achievements">${renderAchievements(ctx)}</div>`))}
...
export function checkAchievements(app) {
    ...
    CONFIG.ACHIEVEMENTS.forEach(ach => {
        if(!app.stats.unlockedAchs.includes(ach.id) && ach.check(ctx, app)) {
...
function renderTitles(app) {
    ...
    return CONFIG.TITLES.map(t => {
        ...
        const badge = isEquipped ? 'EQUIPADO' : isUnlocked ? 'DISPONIBLE' : '🔒';
...
function renderAchievements(ctx) {
    return CONFIG.ACHIEVEMENTS.map(ach => {
        let unlocked = false;
        try { unlocked = !!ach.check(ctx); } catch(e) { /* check roto, ach queda bloqueado */ }
